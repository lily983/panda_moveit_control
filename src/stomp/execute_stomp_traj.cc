...
ExecuteStompTraj::ExecuteStompTraj(ros::NodeHandle n)
  ...
  srv_display_stomp_traj_ = n_.advertiseService(
      "display_stomp_traj", &ExecuteStompTraj::CallBackDisplayStompTraj, this);
...
bool ExecuteStompTraj::CallBackDisplayStompTraj(
  ...
  if (all_stomp_trajectory_.trajectory.size() == 0) {
    ROS_ERROR(
        "Recevied empty stomp trajectories list, failed to display the target "
        "trajectory!");
    return false;
  }
