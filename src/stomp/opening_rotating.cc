...
bool OpeningRotating::CallbackExecuteStompTraj(
  ...
  ROS_INFO(
      "===Received request to execute STOMP trajectory for the OpeningRotating "
      "task!===");
  ...
  traj_stomp_to_pregrasp =
      panda_arm_.PlanningToPoseTarget(grasp_pose_, stomp_first_point.positions);
  ...
  if (traj_start_to_pregrasp.joint_trajectory.points.size() == 0) {
    ROS_ERROR("===Failed to plan path pre-grasp pose joint configuration!===");
    res.success = false;
    return false;
  }
...
void OpeningRotating::GenerateGraspPose(
  ...
  ei_marker_to_grasp = Eigen::Translation3d(Eigen::Vector3d(0.0, 0.0, 0.083)) *
  ...
  if (object == "sink_door") {
    ei_marker_start_to_goal.matrix() << std::cos(angle), -std::sin(angle), 0,
        radius * (1 - std::cos(angle)), std::sin(angle), std::cos(angle), 0,
        -radius * std::sin(angle), 0, 0, 1, 0, 0, 0, 0, 1;
  } else {
