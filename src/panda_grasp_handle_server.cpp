/* This node provide service to control franka arm to grasp the handle of drawer
 * with marker on it.
 */
...
class PandaGraspHandleServer {
  public:
    PandaGraspHandleServer(ros::NodeHandle& nh)
        ...
        // std::cout << "1" << "\n";
        ss_grasp_handle_ = n.advertiseService(
            "panda_grasp_handle",
            &PandaGraspHandleServer::callBackPandaGraspHandle, this);

        // std::cout << "2" << "\n";
        ss_grasp_info_ = n.advertiseService(
            "generate_grasp_info",
            &PandaGraspHandleServer::callBackGenerateGraspInfo, this);

        ss_display_stomp_traj_ = n.advertiseService(
            "display_stomp_traj",
            &PandaGraspHandleServer::callBackDisplayStompTraj, this);

        display_stomp_traj_publisher_ =
            n.advertise<moveit_msgs::DisplayTrajectory>("display_traj", 1);
        ...
        sub_trajectory_ =
            n.subscribe("/move_group/display_planned_path", 1,
                        &PandaGraspHandleServer::callBackTrajectory, this);

        // sub_aruco_marker_ = n.subscribe("/aruco_double/pose2", 1,
        // &PandaGraspHandleServer::callBackAruco, this);

        // sub_obstacle_marker_ = n.subscribe("/aruco_double/pose", 1,
        // &PandaGraspHandleServer::callBackObstacleAruco, this);

        // sub_grasp_pose_ = n.subscribe("/handle_grasp_pose", 1,
        // &PandaGraspHandleServer::CallbackHandleGraspPose, this);

        // rviz_.reset(new rviz_visual_tools::RvizVisualTools("panda_link0",
        // "/rviz_visual_tools"));

        // // Read marker info and publish start pose and goal pose for STOMP
        // planning SlidingGraspInfo();
        ...
        planning_scene_diff_publisher_ =
            n.advertise<moveit_msgs::PlanningScene>("planning_scene", 1);
        ...
        n.getParam("/panda_grasp_handle_server/repo_mesh_path",
                   repo_mesh_path_);
        std::cout << web_mesh_path_ << "\n";
        std::cout << repo_mesh_path_ << "\n";

        std::string web_scene_path, web_obj_path, repo_scene_path,
            repo_obj_path;
    ...
    void RotatingGraspInfo() {
        ...
        ROS_INFO(
            "Finished generating grasp pose, pre-grasp pose, post-grasp pose, "
            "publish to TF tree ......");
        PubGraspPoseToStaticTF();
    }

    void RotatingGenerateGraspPose(
        const geometry_msgs::TransformStamped tf_w2d) {
        ...
        angle = -angle * PI / 180;
        std::cout << "Mode is " << mode << ", radius is " << radius
                  << ", angle is " << angle << "\n";

        T marker_start_to_goal;
        if (object == "sink_door") {
            marker_start_to_goal.matrix() << std::cos(angle), -std::sin(angle),
                0, radius * (1 - std::cos(angle)), std::sin(angle),
                std::cos(angle), 0, -radius * std::sin(angle), 0, 0, 1, 0, 0, 0,
                0, 1;
        } else {
            marker_start_to_goal.matrix() << 1, 0, 0, 0, 0, std::cos(angle),
        ...
        marker_to_grasp =
            Eigen::Translation3d(Eigen::Vector3d(0.0, 0.0, 0.08)) *
            q_marker2grasp.normalized().toRotationMatrix();

        T marker_to_pregrasp;
        marker_to_pregrasp =
            Eigen::Translation3d(Eigen::Vector3d(0.0, 0.0, 0.28)) *
            q_marker2grasp.normalized().toRotationMatrix();

        T marker_to_post_grasp;
        marker_to_post_grasp =
            Eigen::Translation3d(Eigen::Vector3d(0.0, 0.0, 0.08)) *
            q_marker2grasp.normalized().toRotationMatrix();
    ...
    void CallbackHandleGraspPose(const geometry_msgs::PosePtr& msg) {
        ...
        rviz_.reset(new rviz_visual_tools::RvizVisualTools(
            "panda_link0", "/rviz_visual_markers"));
        rviz_->deleteAllMarkers();
        rviz_->publishAxisLabeled(pre_grasp_pose_, "pre_grasp_pose",
                                  rviz_visual_tools::LARGE,
                                  rviz_visual_tools::GREEN);
        rviz_->publishAxisLabeled(grasp_pose_, "grasp_pose",
                                  rviz_visual_tools::LARGE,
                                  rviz_visual_tools::GREEN);
        rviz_->trigger();
    }
    ...
    geometry_msgs::TransformStamped ReadHandleMarker() {
        ...
        try {
            tf_w2d = tf2_buffer_.lookupTransform("panda_link0", "handle_marker",
                                                 ros::Time::now(),
                                                 ros::Duration(3.0));
    ...
    void SlidingGenerateGraspPose(
        const geometry_msgs::TransformStamped tf_w2d) {
        T ei_d2grasp;
        Eigen::Quaterniond q_d2grasp(0.0001, -0.3832, 0.9237, 0.001);
        ei_d2grasp = Eigen::Translation3d(Eigen::Vector3d(-0.01, 0.0, 0.083)) *
                     q_d2grasp.normalized().toRotationMatrix();

        T ei_d2pregrasp;
        ei_d2pregrasp =
            Eigen::Translation3d(Eigen::Vector3d(-0.01, 0.0, 0.183)) *
            q_d2grasp.normalized().toRotationMatrix();
    ...
    void PubGraspPoseToStaticTF() {
        // rviz_->publishAxisLabeled(post_grasp_pose_,"goal_pose",
        // rviz_visual_tools::LARGE, rviz_visual_tools::GREEN);
        // rviz_->publishAxisLabeled(grasp_pose_,"grasp_pose",
        // rviz_visual_tools::LARGE, rviz_visual_tools::GREEN);
        // rviz_->publishAxisLabeled(pre_grasp_pose_,"pre_grasp_pose",
        // rviz_visual_tools::LARGE, rviz_visual_tools::GREEN);
        ...
        tf_post_grasp_pose.transform.translation.x =
            post_grasp_pose_.position.x;
        tf_post_grasp_pose.transform.translation.y =
            post_grasp_pose_.position.y;
        tf_post_grasp_pose.transform.translation.z =
            post_grasp_pose_.position.z;
    ...
    void callBackObstacleAruco(const geometry_msgs::PosePtr& pose_c2o) {
        ...
        try {
            tf_w2c = tf2_buffer_.lookupTransform("panda_link0", "camera_base",
                                                 ros::Time::now(),
                                                 ros::Duration(3.0));
        } catch (tf2::TransformException& ex) {
            ROS_WARN("%s", ex.what());
    ...
    void callBackAruco(const geometry_msgs::PosePtr& pose_c2d) {
        ...
        // std::cout <<  tf_w2d_.transform.rotation.x  <<
        // tf_w2d_.transform.rotation.y  <<tf_w2d_.transform.rotation.z
        // <<tf_w2d_.transform.rotation.w  << "\n";
        publishGraspInfoToMoveit();
    }

    bool callBackGenerateGraspInfo(
        panda_moveit_control::GraspHandleInfo::Request& req,
        panda_moveit_control::GraspHandleInfo::Response& res) {
        ROS_INFO("Received generate grasp info request...");

        try {
            tf_w2d_ = tf2_buffer_.lookupTransform(
                "panda_link0", "aruco_marker_frame", ros::Time::now(),
                ros::Duration(2.0));
        } catch (tf2::TransformException& ex) {
            ROS_WARN("%s", ex.what());
            res.success = 0;
    ...
    void initializedFixedTransformations() {
        ...
        ei_d2L8_ =
            Eigen::Translation3d(Eigen::Vector3d(-0.010, -0.120, 0.053)) *
            q_d2L8.normalized().toRotationMatrix();

        Eigen::Quaterniond q_d2L8_post_grasp(-0.000, 0.925, 0.379, 0.023);
        ei_d2L8_post_grasp_ =
            Eigen::Translation3d(Eigen::Vector3d(-0.010, -0.220, 0.053)) *
            q_d2L8_post_grasp.normalized().toRotationMatrix();

        Eigen::Quaterniond q_d2L8_pre_grasp(-0.000, 0.925, 0.379, 0.023);
        ei_d2L8_pre_grasp_ =
            Eigen::Translation3d(Eigen::Vector3d(-0.010, -0.120, 0.3)) *
            q_d2L8_pre_grasp.normalized().toRotationMatrix();
    }

    // Receive the robot trajectory planned by STOMP, use that as the second
    // executed trajectory Planning a grasping trajectory based on the start
    // joint configuration of the second executed trajectory
    ...
    bool callBackPandaGraspHandle(
        panda_moveit_control::PandaGraspHandle::Request& req,
        ...
        if (move_group_.plan(my_plan) ==
            ...
            moveit_visual_tools_.publishTrajectoryLine(my_plan.trajectory_,
                                                       joint_model_group);
            ...
            ROS_ERROR(
                "Failed to plan from arm start configuration to "
                "grasp_pose_joint_values");
            res.success = 0;
            return 0;
        }
    ...
    /*
        bool
       callBackPandaGraspHandle(panda_moveit_control::PandaGraspHandle::Request&
       req, panda_moveit_control::PandaGraspHandle::Response& res)
        {
       ...
       move_group_.getCurrentState();
            ...
            moveit::core::RobotStatePtr stomp_start_state =
       move_group_.getCurrentState();

            std::vector<double> joint_group_positions;
            joint_group_positions = stomp_start_point.positions;

            stomp_start_state->setJointGroupPositions(joint_model_group,
       joint_group_positions);

            move_group_.setStartState(*stomp_start_state);
            ROS_INFO("Reverse planning, set start state to the first stomp joint
       trajectory point ");
       ...
       moveit::planning_interface::MoveItErrorCode::SUCCESS)
            {
                pre_grasp_pose_joint_values =
       my_plan.trajectory_.joint_trajectory.points.at(my_plan.trajectory_.joint_trajectory.points.size()
       - 1).positions; ROS_INFO("Successfully planned reverse planning");
                moveit_visual_tools_.publishTrajectoryLine(my_plan.trajectory_,
       joint_model_group); moveit_visual_tools_.trigger();
                // move_group_.execute(my_plan.trajectory_);
            }
            else{
            ...
            ROS_INFO("Start plan from arm start configuration to
       pre_grasp_pose_joint_values");
       ...
       moveit::planning_interface::MoveItErrorCode::SUCCESS)
            {
                ...
                ROS_INFO("Successfully planned from arm start configuration to
       pre_grasp_pose_joint_values");
                moveit_visual_tools_.publishTrajectoryLine(my_plan.trajectory_,
       joint_model_group); moveit_visual_tools_.trigger();
                // move_group_.execute(my_plan.trajectory_);
                ...
                ROS_ERROR("Failed to plan from arm start configuration to
       pre_grasp_pose_joint_values"); res.success = 0; return 0;
            }

            moveit::core::RobotStatePtr pre_grasp_finished_state =
       move_group_.getCurrentState();
            pre_grasp_finished_state->setJointGroupPositions(joint_model_group,
       pre_grasp_pose_joint_values);
       ...
       joint_model_group); moveit_visual_tools_.trigger();
                ...
                ROS_ERROR("Failed to plan from arm start configuration to
       grasp_pose_joint_values"); res.success = 0; return 0;
            }


            moveit_visual_tools_.publishTrajectoryLine(pre_grasp_trajectory_,
       joint_model_group);
            moveit_visual_tools_.publishTrajectoryLine(pre_grasp_2_grasp_trajectory_,
       joint_model_group);
            moveit_visual_tools_.publishTrajectoryLine(stomp_trajectory_,
       joint_model_group); moveit_visual_tools_.trigger();
    ...
    */
    ...
    // When receive the request to grasp the handle of the drawer, service
    // callback function listen to the drawer marker pose and convert it to the
    // target grasp message(contained pre-grasp, grasp, post-grasp info). Then
    // it calls the arm to execute the grasp
    /*
        bool
    callBackPandaGraspHandle(panda_moveit_control::PandaGraspHandle::Request&
    req, panda_moveit_control::PandaGraspHandle::Response& res)
    ...
    .........");
            ...
            stomp_trajectory_ = all_stomp_trajectory_.trajectory.at(req.num);
            std::cout << "STOMP traj number is " <<
    stomp_trajectory_.joint_trajectory.points.size() << "\n";
    ...
    moveit::planning_interface::MoveItErrorCode::SUCCESS)
            {
                move_group_.execute(my_plan.trajectory_);
                ROS_INFO("Reached start point, hand over
    cup!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1");
            }
            else{
                ROS_ERROR("Failed to plan from arm start configuration to stomp
    first points"); res.success = 0; return 0;
            }
    ...
    */
    ...
    void generateGraspInfo(geometry_msgs::TransformStamped T_w2d) {
        ...
        // pre-grasp pose is higher than grasp pose in z-axis relative to world
        // frame
        ...
        rviz_->publishAxisLabeled(pose_w2d, "drawer_pose",
                                  rviz_visual_tools::LARGE,
                                  rviz_visual_tools::GREEN);
        rviz_->publishAxisLabeled(pre_grasp_pose_, "grasp_pose",
                                  rviz_visual_tools::LARGE,
                                  rviz_visual_tools::GREEN);
        rviz_->publishAxisLabeled(grasp_pose_, "grasp_pose",
                                  rviz_visual_tools::LARGE,
    ...
    void printPose(geometry_msgs::Pose pose) {
        ROS_INFO("The position xyz is: [%f, %f, %f]", pose.position.x,
                 pose.position.y, pose.position.z);
        ROS_INFO("The quaternion xyzw is: [%f, %f, %f, %f]", pose.orientation.x,
    ...
    void addCollisionObjects() {
        ...
        // obstacle_pose_ptr =
        // ros::topic::waitForMessage<geometry_msgs::Pose>("/aruco_double/pose",
        // ros::Duration(10.0)); tf_w2o_.transform.translation.x =
        // obstacle_pose_ptr->position.x; tf_w2o_.transform.translation.y =
        // obstacle_pose_ptr->position.y; tf_w2o_.transform.translation.z =
        // obstacle_pose_ptr->position.z; tf_w2o_.transform.rotation =
        // obstacle_pose_ptr->orientation;
        ...
        collision_objects[1].primitive_poses[0].position.x =
            tf_w2o_.transform.translation.x;
        collision_objects[1].primitive_poses[0].position.y =
            tf_w2o_.transform.translation.y;
        collision_objects[1].primitive_poses[0].position.z =
            tf_w2o_.transform.translation.z - 0.195;
        collision_objects[1].primitive_poses[0].orientation.w =
            tf_w2o_.transform.rotation.w;
        collision_objects[1].primitive_poses[0].orientation.x =
            tf_w2o_.transform.rotation.x;
        collision_objects[1].primitive_poses[0].orientation.y =
            tf_w2o_.transform.rotation.y;
        collision_objects[1].primitive_poses[0].orientation.z =
            tf_w2o_.transform.rotation.z;
    ...
    void addCollisionDrawer() {
        ...
        collision_drawer.primitives[0].type =
            collision_drawer.primitives[0].BOX;
        ...
        collision_drawer.primitive_poses[0].position.x =
            tf_w2d_.transform.translation.x;
        collision_drawer.primitive_poses[0].position.y =
            tf_w2d_.transform.translation.y;
        collision_drawer.primitive_poses[0].position.z =
            tf_w2d_.transform.translation.z - 0.135;
    ...
    void AddCollisionSceneMesh() {
        ...
        shapes::Mesh* scene_mesh = shapes::createMeshFromResource(
    ...
    void AddCollisionObjMesh() {
        ...
        shapes::Mesh* obj_mesh = shapes::createMeshFromResource(
        ...
        shape_msgs::Mesh obj_mesh_moveit =
            boost::get<shape_msgs::Mesh>(obj_mesh_msg);
    ...
    bool callBackDisplayStompTraj(
        ...
        display_traj.trajectory.push_back(
            all_stomp_trajectory_.trajectory.at(req.num));
  ...
  protected:
    ...
    geometry_msgs::TransformStamped
        tf_w2o_;  // Obstacle pose in the world frame
